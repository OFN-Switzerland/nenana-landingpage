	...
	return (
		<>
			<H1 className="card-title">
				{t('registerStart.title', "Already registered with Ne'Na'Na?")}
			</H1>
			<P>
				{t(
					'registerStart.p1',
			...
			<Alert className="flex w-full flex-col">
				<div className="flex w-full flex-col gap-6 md:flex-row">
					<Link className="w-full" to={href('/:lang/registration/form', { lang })}>
						<Button
							className="w-full"
							onClick={() => void plausibleClientEvent({ name: RegistrationEvents.FormStart })}>
					...
					<Link className="w-full" to={href('/:lang/registration/completed', { lang })}>
						<Button
							className="w-full"
							onClick={() =>
								void plausibleClientEvent({ name: RegistrationEvents.SkipRegistration })
							}
							variant="outline">
						...
						<legend className="mt-2 text-center">
							{t(
								'registerStart.actions.continueLegend',
								'For existing customer with an OFN account',
							)}
						</legend>
					</Link>
				</div>
